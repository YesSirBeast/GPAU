...
class DbFile:
    mapping = {
        ...
        "leaderboard_pending_scores": LeaderboardPendingOp,
    ...
    def remove_duplicate_lb_pending_ops(self, by_col: str="external_leaderboard_id"):
        ops = [LeaderboardPendingOp(*x) for x in self.ex("leaderboard_pending_scores").fetchall()]
        seen = set()
        removed = 0
        for op in ops:
            if getattr(op, by_col) in seen:
                self.cur.execute("delete from leaderboard_pending_scores where _id = ?", (op.id,))
                removed += 1
            else:
                seen.add(getattr(op, by_col))
        self.connection.commit()
    ...
    def empty_lb_pending_ops(self):
        self.cur.execute("delete from leaderboard_pending_scores")
        self.connection.commit()

    def add_lb_pending_op(self, op: Dict[str, Any]):
        if "achieved_timestamp" not in op or not op["achieved_timestamp"]:
            op["achieved_timestamp"] = int(time.time() * 1000)
        cols = ",".join(op.keys())
        placeholders = ",".join("?" for _ in range(len(op)))
        sql = f"insert into leaderboard_pending_scores ({cols}) values ({placeholders})"
        try:
            self.cur.execute(sql, list(op.values()))
            self.connection.commit()
        except Exception as e:
            print(f"[!] Warning: Failed to insert pending op: {e}")

    def get_next_lb_pending_op_id(self):
        res = self.cur.execute("select max(_id) from leaderboard_pending_scores").fetchone()[0]
        return 0 if res is None else res + 1
