...
class LeaderboardPendingOp(Wrapper):
    def __init__(self, *args) -> None:
        ...
        self._id = self.get_arg(0, args)
        self.client_context_id = self.get_arg(1, args)
        self.external_game_id = self.get_arg(2, args)
        self.external_leaderboard_id = self.get_arg(3, args)
        self.external_player_id = self.get_arg(4, args)
        self.score = self.get_arg(5, args)
        self.achieved_timestamp = self.get_arg(6, args)
        self.score_tag = self.get_arg(7, args)
        self.signature = self.get_arg(8, args)
...
class Finder:
    ...
    def game_by_lb_def(self, x: LeaderboardDefinition) -> Optional[Game]:
        return self.game_by_id(x.game_id) if x else None

    def game_by_lb_inst(self, x: LeaderboardInstance) -> Optional[Game]:
        lb_def = self.db.select_by_cls(LeaderboardDefinition, values=[x.leaderboard_id], first=True)
        return self.game_by_lb_def(lb_def) if lb_def else None
